...
export function actualizarGotas(object, time, numblobs, floor, wallx, wallz) {
  ...
  for (let i = 0; i < numblobs; i++) {
    ...
    // } else {
    let indiceColor = Math.round(mapRange(ballx, 0.2, 0.8, 0, rainbow.length - 1));
    let color = rainbow[indiceColor % 7];
    // console.log(indiceColor, ballx, color);
    object.addBall(ballx, bally2, ballz, strength * 0.4 * multStrg, subtract, color);
    object.addBall(ballx, bally, ballz, strength * multStrg, subtract, color);
    // }
  }
  console.log(maximo);
  let c = 10;
